 ...
 * green counts half, red counts for nothing. With no history yet the
 * coach starts at a neutral midpoint rather than looking unwell.
 */
export function computeFitness(entries: DayEntry[]): number {
  const recent = entries.slice(-RECENT_WINDOW);
  if (recent.length === 0) return 0.5;

  const weight = { gold: 1, green: 0.55, red: 0 } as const;
